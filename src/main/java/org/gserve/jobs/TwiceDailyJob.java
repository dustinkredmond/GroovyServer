...
public class TwiceDailyJob implements Job {
    @Override
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        try {
            List<GroovyScript> gs = GroovyScript.findActive();
            gs.forEach(script -> {
                if (script.getSchedule().equalsIgnoreCase("Twice Daily")){
                    GroovyScriptRunner.execute(script.getClassName(),script.getCode());
                }
            });
        } catch (Exception e) {
            Logger log = new Logger();
            log.logError("An exception occurred at org.gserve.jobs.TwiceDailyJob.execute() | "+e.getMessage());
        }
